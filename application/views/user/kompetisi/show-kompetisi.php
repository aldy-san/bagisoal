<div class="container p-3 my-3 rounded bg-white shadow">
    <?= !$this->m_kompetisi->cekTerdaftar($this->session->userdata('id_user'), $this->uri->segment(3)) ? "<a href=" . base_url('kompetisi/daftar/') .  $this->uri->segment(3) . " class='btn btn-primary float-right'>Daftar</a>" : ''; ?>
        ...
        <table class="table text-center">
            ...
            <tbody>
                ...
                foreach ($pendaftar as $pendaftar) : ?>
                    <tr>
                        <th scope="row"><?= ++$no ?></th>
                        <td><a href="<?= base_url('user/profile/') ?><?= $pendaftar['id_user'] ?>"><?= $pendaftar['nama'] ?></a></td>
